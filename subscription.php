<?php 
    ...
    if(isset($_POST['subscribe'])){ //if subscribe btn clicked
      ...
      if(filter_var($userEmail, FILTER_VALIDATE_EMAIL)){ //validating user email
        $subject = "Thanks for Subscribing us - CoHelp";
        ...
        $sender = "From: contact.cohelp@gmail.com";

        $code = rand(999999, 111111);
        $status = "notverified";
        $insert_data = "INSERT INTO subscribertable (email, code, status)
                        values('$userEmail', '$code', '$status')";
